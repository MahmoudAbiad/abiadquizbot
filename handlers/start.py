...
async def start(msg: types.Message, command: CommandObject, state: FSMContext):
    try:
        ...
        welcome_text += (
            ...
            f"<code>• 1 نقطة لكل صفحة PDF أو صورة (حتى 15 صفحة)</code>\n"
            f"<code>• 1 نقطة لكل سؤال يتم توليده (حتى 30 سؤالاً)</code>\n"
            f"<code>• 1.5 نقطة للصفحات والأسئلة الإضافية/الكبيرة</code>\n"
            f"<code>• ⚡ خصم 90% عند فتح كويز جاهز تم توليده سابقاً!</code>\n\n"
